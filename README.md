...
- **Runtime sensitivity estimation for scale-free kNN in PuzirCalibrator** — currently, `p_range = p_max - p_min` is used to normalise parameter distances before kNN neighbourhood search. This means the quality of neighbourhood structure (and therefore local covariance estimation and ridge/CMA drift) depends on how accurately the user-specified bounds reflect the true sensitivity of the loss function to each parameter. For general models with many parameters and unclear physics, this prior can be wrong. A better approach: during `memory_probation` (where random parameter batches are already evaluated), fit a shallow random forest on `(params → score)` and derive per-parameter sensitivity from feature importances or leaf co-occurrence proximity. Use this runtime-estimated metric for kNN normalisation in `refine`, falling back to `p_range` if history is too small. This would make `calibrate_radii` a pure guardrail (preventing wasted compute on irrelevant regions) rather than a calibration-critical tuning knob.

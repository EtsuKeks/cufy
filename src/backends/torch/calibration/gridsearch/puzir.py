...
from src.backends.torch.calibration.gridsearch.base import GridSearchModel, GridSearchModelParam, GridSearchSettings
...
class PuzirModel(GridSearchModel, ABC):
    def __init__(
        self, param_overrides: dict[str, GridSearchModelParam] | None = None, gs: PuzirSettings | None = None
    ):
    ...
    def refine(self, *, p_min: torch.Tensor, p_max: torch.Tensor, data: Batch1D) -> None:
        ...
        d = settings.device
        dt = settings.dtype
        eps = torch.tensor(settings.ppl.epsilon, device=d, dtype=dt)
        ...
        if self._param_batch_size is not None:
            self.save_fact("refined", "batch_capacity", str(self._param_batch_size))

        if self.gs.initial_sampler == "sobol":
            self.save_fact("refined", "budget_batches", str(self.gs.refine_multiplier))
        else:
            self.save_fact("refined", "budget_points", str(self._search_points_detailed))

        p_range = (p_max - p_min).clamp_min(eps)
        if self.gs.initial_sampler == "grid":
            denom = torch.tensor([max(x - 1, 1) for x in self._search_points_detailed], device=d, dtype=dt)
            cell = p_range / denom
        else:

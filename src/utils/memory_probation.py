...
def probe_param_batch_sizes(
    *, make_f: Callable[[int, int], Callable[[], None]], max_n: int, max_m: int, available_memory_fraction: float
    ...
    X = torch.tensor([[1.0, n, m, m * n] for (m, n, _) in samples], dtype=settings.dtype)
    y = torch.tensor([p for (_, _, p) in samples], dtype=settings.dtype).reshape(-1, 1)
    ...
    print(
        f"[memory_probation] probe: device={settings.device.type}, {max_n=}, {max_m=}, {available_bytes=}.\n"
    ...
    def batch_size(n: int) -> int:
        a = a0 + a1 * max(1, n)
        b = b0 + b1 * max(1, n)
        return max(1, (available_bytes - a) // b)
